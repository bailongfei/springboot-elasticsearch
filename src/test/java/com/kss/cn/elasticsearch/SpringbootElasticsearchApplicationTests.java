package com.kss.cn.elasticsearch;

import com.alibaba.fastjson.JSON;
import com.kss.cn.elasticsearch.pojo.User;
...
class SpringbootElasticsearchApplicationTests {
    ...
    @Test
    public void tetsCreateIndex() throws IOException {
    ...
    public void testDeleteIndex() throws IOException {
        DeleteIndexRequest deleteIndexRequest = new DeleteIndexRequest("liuyou_index");
    ...
    public void testAddDocument() throws IOException {
        // 创建一个User对象
        User liuyou = new User("liuyouzhandb", 18);
        // 创建请求索引
        IndexRequest request = new IndexRequest("liuyou_index");
        // 制定规则 PUT /liuyou_index/_doc/1
        request.id("1003");// 设置文档ID
        request.timeout(TimeValue.timeValueMillis(1000));// 超时时间request.timeout("1s")
        // 将我们的数据放入请求中
    ...
    //测试获得文档信息
    @Test
    public void testGetDecument() throws IOException {
        GetRequest request = new GetRequest("liuyou_index", "1001");
    ...
    public void testUpdateDocument() throws IOException {
        UpdateRequest request = new UpdateRequest("liuyou_index", "1001");
        User lmk = new User("战术改善", 11);
        request.doc(JSON.toJSONString(lmk), XContentType.JSON);
    ...
    public void testDeleteDocument() throws IOException {
        DeleteRequest request = new DeleteRequest("liuyou_index", "1001");
        request.timeout("1s");
    ...
    //前面的操作都无法批量添加数据
    //上面的这些api无法批量增加数据,下面方法（只会保留最后一个source）kai
    @Test
    public void testbatchto1() throws IOException {
    ...
    //特殊的，真的项目一般会 批量插入数据
    @Test
    public void TestBatchInstall() throws IOException {
        ...
        for (int i = 0; i < users.size(); i++) {
            bulkRequest.add(
                    ...
                    new IndexRequest("bulk").
                            id("bulkid_"+(i+1))// 没有设置id 会自定生成一个随机id
        ...
        System.out.println(bulk.isFragment());//是否出错
